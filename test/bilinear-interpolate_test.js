...
var bilinearInterpolate = require('../');
...
describe('bilinearInterpolate', function () {
  ...
  var line = bilinearInterpolate(d);

    it(' should find exact values', function() {
      assert.equal(line(4), 45.5);
      assert.equal(line(5), 44.6);
    });

    it(' should interpolate', function() {
      assert.equal(line(4.5), 45.05);
    });
});
